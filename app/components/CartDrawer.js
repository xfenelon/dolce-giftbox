...
export default function CartDrawer({ open, onClose }) {
  ...
  const formattedTotal = `$${totalPrice.toLocaleString("es-CO")}`;
    ...
    .map((i) => `- ${i.name}${i.ribbon ? ` (listón ${i.ribbon})` : ""}${i.variant ? ` (${i.variant})` : ""}${(i.itemOptions || []).map((o) => ` (${o.label}: ${o.value})`).join("")}${i.customName ? ` (nombre: ${i.customName})` : ""} x${i.qty}`)
    .join("%0A");
      ...
      <div className="cart-drawer">
        ...
        {items.length === 0 ? (
          ...
          <div className="cart-items">
            {items.map((item) => (
                                                <div className="cart-item" key={`${item.slug}-${item.ribbon}-${item.variant}-${item.customName}-${JSON.stringify(item.itemOptions)}`}>
                ...
                <div className="cart-item-info">
                  ...
                  {item.ribbon && <p className="cart-item-ribbon">Listón: {item.ribbon}</p>}
                        ...
                        {(item.itemOptions || []).map((o) => (
                    <p key={o.label} className="cart-item-ribbon">{o.label}: {o.value}</p>
                  ))}
                  {item.customName && <p className="cart-item-ribbon">Nombre: {item.customName}</p>}
                  <div className="cart-item-controls">
                    <div className="cart-item-qty">
                      <button onClick={() => updateQty(item.slug, item.ribbon, item.qty - 1, item.variant, item.customName, item.itemOptions)} aria-label="Menos"><Minus size={12} /></button>
                      <span>{item.qty}</span>
                      <button onClick={() => updateQty(item.slug, item.ribbon, item.qty + 1, item.variant, item.customName, item.itemOptions)} aria-label="Más"><Plus size={12} /></button>
                    </div>
                    <button className="cart-item-remove" onClick={() => removeItem(item.slug, item.ribbon, item.variant, item.customName, item.itemOptions)} aria-label="Quitar">
                      <Trash2 size={16} />
                    </button>
                  </div>
