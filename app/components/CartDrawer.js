...
export default function CartDrawer({ open, onClose }) {
  ...
  const formattedTotal = `$${totalPrice.toLocaleString("es-CO")}`;
   ...
   const whatsappMessage = items
    .map((i) => `- ${i.name}${i.ribbon ? ` (listón ${i.ribbon})` : ""}${i.variant ? ` (${i.variant})` : ""}${i.itemOption ? ` (${i.itemOptionLabel}: ${i.itemOption})` : ""}${i.customName ? ` (nombre: ${i.customName})` : ""} x${i.qty}`)
    .join("%0A");
      ...
      <div className="cart-drawer">
        ...
        {items.length === 0 ? (
          ...
          <div className="cart-items">
            {items.map((item) => (
                           <div className="cart-item" key={`${item.slug}-${item.ribbon}-${item.variant}-${item.customName}-${item.itemOption}`}>
                ...
                <div className="cart-item-info">
                  ...
                  {item.itemOption && <p className="cart-item-ribbon">{item.itemOptionLabel}: {item.itemOption}</p>}
                  {item.customName && <p className="cart-item-ribbon">Nombre: {item.customName}</p>}
                  <div className="cart-item-controls">
                    <div className="cart-item-qty">
                      <button onClick={() => updateQty(item.slug, item.ribbon, item.qty - 1, item.variant, item.customName, item.itemOption)} aria-label="Menos"><Minus size={12} /></button>
                      <span>{item.qty}</span>
                      <button onClick={() => updateQty(item.slug, item.ribbon, item.qty + 1, item.variant, item.customName, item.itemOption)} aria-label="Más"><Plus size={12} /></button>
                    </div>
                    <button className="cart-item-remove" onClick={() => removeItem(item.slug, item.ribbon, item.variant, item.customName, item.itemOption)} aria-label="Quitar">
                      <Trash2 size={16} />
                    </button>
                  </div>
